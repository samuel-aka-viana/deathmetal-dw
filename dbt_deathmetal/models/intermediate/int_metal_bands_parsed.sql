...
simple_parsing as (
    select
        *,
        ...
        case
            when clean_active = 'N/A' then null
            when periods_count = 3 and period_3 is not null then
                case
                    when period_3 like '%present%' then null
                    when period_3 like '%?%' then null
                    when period_3 like '____-____' then cast(right(period_3, 4) as integer)
                    when period_3 like '?-____' then cast(right(period_3, 4) as integer)
                    else null
                end
            when periods_count = 2 and period_2 is not null then
                case
                    when period_2 like '%present%' then null
                    when period_2 like '%?%' then null
                    when period_2 like '____-____' then cast(right(period_2, 4) as integer)
                    when period_2 like '?-____' then cast(right(period_2, 4) as integer)
                    else null
                end
            when periods_count = 1 and period_1 is not null then
                case
                    when period_1 like '%present%' then null
                    when period_1 like '%?%' then null
                    when period_1 like '____-____' then cast(right(period_1, 4) as integer)
                    when period_1 like '?-____' then cast(right(period_1, 4) as integer)
                    else null
                end
            else null
        end as end_year
...
select
    ...
    case when original_active = 'N/A' then true else false end as is_na_case
